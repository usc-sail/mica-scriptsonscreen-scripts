...
movie_scripts_dir = os.path.join(os.getenv("DATA_DIR"), "mica-movie-scripts/scriptsonscreen/download")

# define command-line flags
# --movie_scripts_dir to specify a different movie_scripts_dir
FLAGS = flags.FLAGS
flags.DEFINE_string("scripts", default=movie_scripts_dir, help="Movie Scripts directory.")
flags.DEFINE_bool("overwrite", default=False, help="Overwrite existing data.")

# custom json encoder for imdb movie
...
def find_imdb_movie_data(_):
    imdb_movie_data_file = os.path.join(FLAGS.scripts, "imdb_id_to_movie.json")
    error_imdb_ids_file = os.path.join(FLAGS.scripts, "error_imdb_ids.txt")
    movie_script_index_file = os.path.join(FLAGS.scripts, "index.csv")
    ...
    if not FLAGS.overwrite and os.path.exists(imdb_movie_data_file):
